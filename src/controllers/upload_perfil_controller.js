...
const storage_perfil = multer.diskStorage({
    ...
    */
    destination: function (req, file, cb) {
        ...
        console.log('llego a la funcion de copiar archivo de perfil')

        if (file_type === '.mp4') {
            cb(null, process.env.HOME + '/publicaciones/perfil/videos')
        }

        if (file_type === '.jpg' || file_type === '.png' || file_type === '.jpeg') {
            cb(null, process.env.HOME + '/publicaciones/perfil')
        }
    },
    ...
    */
    filename: function (req, file, cb) {
        ...
        if (file_type === '.mp4') {
            file_type = '.mp4'
            file_name = 'vid_perfil_';
        }

        if (file_type === '.jpg' || file_type === '.png' || file_type === '.jpeg') {
            file_type = '.jpg'
            file_name = 'img_perfil_';
        }
